...
pub fn eliminate_dead_code(code: &mut Code<'_>) -> bool {
    ...
    for block in code.blocks.iter() {
        for inst in block.insts.iter() {
            if !handle_inst(code, &mut live_tmps, &mut live_stack_slots, inst, &mut changed) {
    ...
    let run_forward = |code: &Code,
                           live_tmps: &mut TmpSet,
                           live_stack_slots: &mut IndexSet<StackSlotId>,
                           changed: &mut bool, possibly_dead: &mut Vec<&Inst>|
     -> bool {
        *changed = false;
     ...
     -> bool {
        *changed = false;
        for i in (0..possibly_dead.len()).rev() {
            let result = handle_inst(code, live_tmps, live_stack_slots, &possibly_dead[i], changed);
    ...
    loop {
        ...
        if !run_backward(code, &mut live_tmps, &mut live_stack_slots, &mut changed, &mut possibly_dead) {
            break;
        }

        if !run_forward(code, &mut live_tmps, &mut live_stack_slots, &mut changed, &mut possibly_dead) {
            break;
        }
    ...
    let code2 = unsafe {
        &mut *(code as *const Code as *mut Code)
    };
