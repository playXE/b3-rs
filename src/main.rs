...
use b3::jit::reg::Reg;
use b3::{self, BasicBlockBuilder, Frequency};
...
impl BfJIT {
    ...
    fn do_translate(&self, disasm: bool, input: &[Token]) -> Compilation {
        let mut proc = b3::Procedure::new(b3::Options::default());

        let mut entry = proc.add_block(1.0);
        let mut exit = proc.add_block(1.0);
        ...
        for t in input {
            match *t {
                Token::Forward(n) => {
                    ...
                    let n = builder.const64(n as i64);
                ...
                Token::Add(n) => {
                    ...
                    let n = builder.const32(n as i32);
